...
class WorkspaceMemberRepository {
    async create(fk_id_workspace, fk_id_user, role, acceptInvitation = 'pending') {
        try {
    ...
    async deleteByWorkspaceId(workspace_id) {
        try {
            ...
            throw new ServerError("Error al eliminar los miembros del espacio", 500);
        }
    }
    ...
    async getWorkspaceListByUserId(user_id) {
        try {
            ...
            const members_mapped = members
                .filter(member => member.fk_id_workspace !== null)
                .map(
                    (member) => {
                        return {
                            member_id: member._id,
